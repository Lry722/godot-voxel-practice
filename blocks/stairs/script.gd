...
func update(pos : Vector3i, from : Vector3i, tool : VoxelToolTerrain) -> bool:
	...
	var orientation := Util.get_orientation_by_name(Blocks.get_variant_model_by_index(variant_index).resource_name)
	...
	if from_block_index != index:
		tool.set_voxel(pos, Blocks.get_variant_by_name('stairs_default_' + orientation))
		return false
	
	var from_variant_orientation := Util.get_orientation_by_name(Blocks.get_variant_model_by_index(from_variant_index).resource_name)
	...
	if -orientation_vector == from:
		...
		if orientation.contains('D'):
			cross.y *= -1
		if cross.y > 0:
			tool.set_voxel(pos, Blocks.get_variant_by_name('stairs_leftDot_' + orientation))
		elif cross.y < 0:
			tool.set_voxel(pos, Blocks.get_variant_by_name('stairs_rightDot_' + orientation))
		else:
			return false
